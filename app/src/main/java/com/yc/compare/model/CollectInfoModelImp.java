...
public class CollectInfoModelImp extends BaseModel implements CollectInfoModel<CollectInfoRet> {
    ...
    @Override
    public void historyList(String uid, int page, final IBaseRequestCallBack<CollectInfoRet> iBaseRequestCallBack) {
        JSONObject params = new JSONObject();
        try {
            params.put("uid", uid);
            params.put("page", page);
        ...
        RequestBody requestBody = RequestBody.create(MediaType.parse("application/json"), params.toString());
        mCompositeSubscription.add(collectInfoServiceApi.historyList(requestBody)  //将subscribe添加到subscription，用于注销subscribe
                .observeOn(AndroidSchedulers.mainThread())//指定事件消费线程
                .subscribeOn(Schedulers.io())  //指定 subscribe() 发生在 IO 线程
                .subscribe(new Subscriber<CollectInfoRet>() {
